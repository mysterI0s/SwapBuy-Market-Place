...
class CartSwapPageController with ChangeNotifier {
  ProductFull? productrequest;
  ProductFull? productoffered;

  initstate(ProductFull productrequest, ProductFull productoffered) {
  ...
  Future<Either<Failure, bool>> RequestSwap(BuildContext context) async {
    ...
    try {
      var response = await client.request(
        path: AppApi.RequestSwap(
          Provider.of<ServicesProvider>(context, listen: false).userid,
          productrequest!.product!.id!,
        ),
